import 'package:flutter/material.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:nanoid/nanoid.dart';
import '../../../data/models/user_model.dart';
import '../../../services/question_service.dart';
import '../../../services/asset_service.dart';
import '../../../services/ad_service.dart';
import '../../../data/models/ad_log_model.dart';
import 'dart:io';
import 'package:path_provider/path_provider.dart';
import '../../../data/models/diary_model.dart';
import '../../../data/models/version_model.dart';
import '../../../utils/encryption.dart';

class AdminController extends ChangeNotifier {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final AssetService _assetService = AssetService();
  final QuestionService _questionService = QuestionService();
  final AdService _adService = AdService();
  final String? _currentUserEmail;

  String? get currentUserEmail => _currentUserEmail;

  AdminController(this._currentUserEmail);

  Future<void> assignReferralCodesToOldUsers() async {
    _isLoading = true;
    notifyListeners();
    try {
      final snapshot = await _firestore.collection('users').get();
      int updatedCount = 0;
      WriteBatch batch = _firestore.batch();

      for (var doc in snapshot.docs) {
        final data = doc.data();
        Map<String, dynamic> updates = {};

        // 1. 추천인 코드가 없는 경우 생성
        if (data['referralCode'] == null ||
            (data['referralCode'] as String).isEmpty) {
          updates['referralCode'] =
              customAlphabet('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', 8);
        }

        // 2. 기존 유저들의 경우 닉네임 설정 화면이 뜨지 않도록 설정 완료 상태로 변경
        if (data['isSetupComplete'] != true) {
          updates['isSetupComplete'] = true;
        }

        if (updates.isNotEmpty) {
          batch.update(doc.reference, updates);
          updatedCount++;

          if (updatedCount % 400 == 0) {
            await batch.commit();
            batch = _firestore.batch();
          }
        }
      }

      if (updatedCount > 0 && updatedCount % 400 != 0) {
        await batch.commit();
      }

      debugPrint('Assigned referral codes to $updatedCount users.');
    } catch (e) {
      debugPrint('Error assigning referral codes: $e');
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  Future<void> syncShopAssets() async {
    _isLoading = true;
    notifyListeners();
    try {
      await _assetService.migrateLocalAssetsToFirestore();
      await _assetService.fetchDynamicAssets(); // 동기화 후 최신 데이터 다시 불러오기
      debugPrint('Shop assets synchronized successfully.');
    } catch (e) {
      debugPrint('Error syncing shop assets: $e');
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  }

  static const List<String> _adminEmails = [
    'admin@morningmate.com',
    'admin@test.com'
  ];

  bool _isLoading = false;
  bool get isLoading => _isLoading;

  List<Map<String, dynamic>> _reports = [];
  List<Map<String, dynamic>> get reports => _reports;

  bool get isAdmin {
    if (_currentUserEmail == null) return false;
    return _adminEmails.contains(_currentUserEmail);
  }

  bool _isDisposed = false;

  @override
  void dispose() {
    _isDisposed = true;
    super.dispose();
  }

  Future<void> fetchReports() async {
    if (_isDisposed) return;
    _isLoading = true;
    notifyListeners();

    try {
      final snapshot = await _firestore
          .collection('reports')
          .orderBy('createdAt', descending: true)
          .get();

      if (_isDisposed) return;

      _reports = snapshot.docs.map((doc) {
        final data = doc.data();
        data['id'] = doc.id;
        if (data['createdAt'] is Timestamp) {
          data['createdAt'] = (data['createdAt'] as Timestamp).toDate();
        }
        return data;
      }).toList();
    } catch (e) {
      debugPrint('신고 목록 가져오기 오류: $e');
    }

    if (_isDisposed) return;
    _isLoading = false;
    notifyListeners();
  }

  Map<String, int> _shopDiscounts = {};
  Map<String, int> get shopDiscounts => _shopDiscounts;

  Future<void> fetchShopDiscounts() async {
    if (_isDisposed) return;
    try {
      final doc = await _firestore.collection('settings').doc('shop').get();
      if (doc.exists && doc.data() != null) {
        final data = doc.data()!;
        if (data['discounts'] != null) {
          _shopDiscounts = Map<String, int>.from(data['discounts']);
        }
      } else {
        _shopDiscounts = {};
      }
    } catch (e) {
      debugPrint('할인 정보 불러오기 오류: $e');
    }
    if (_isDisposed) return;
    notifyListeners();
  }

  Future<void> setShopDiscount(String itemId, int price) async {
    _isLoading = true;
    notifyListeners();
    try {
      _shopDiscounts[itemId] = price;
      await _firestore.collection('settings').doc('shop').set({
        'discounts': _shopDiscounts,
      }, SetOptions(merge: true));
    } catch (e) {
      debugPrint('할인 설정 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  Future<void> removeShopDiscount(String itemId) async {
    _isLoading = true;
    notifyListeners();
    try {
      _shopDiscounts.remove(itemId);
      await _firestore.collection('settings').doc('shop').set({
        'discounts': _shopDiscounts,
      }, SetOptions(merge: true));
    } catch (e) {
      debugPrint('할인 해제 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  Future<void> removeAllShopDiscounts() async {
    _isLoading = true;
    notifyListeners();
    try {
      _shopDiscounts.clear();
      await _firestore.collection('settings').doc('shop').set({
        'discounts': {},
      }, SetOptions(merge: true));
    } catch (e) {
      debugPrint('모든 할인 제거 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  Future<void> deleteNote(String reportId, String targetUserId, String noteId,
      String reporterId) async {
    _isLoading = true;
    notifyListeners();

    try {
      await _firestore.runTransaction((transaction) async {
        final userRef = _firestore.collection('users').doc(targetUserId);
        final userSnapshot = await transaction.get(userRef);

        if (userSnapshot.exists) {
          final userData = userSnapshot.data()!;
          if (userData['roomDecoration'] != null) {
            final decorationMap =
                Map<String, dynamic>.from(userData['roomDecoration']);
            final props =
                List<Map<String, dynamic>>.from(decorationMap['props'] ?? []);
            props.removeWhere((prop) => prop['id'] == noteId);
            decorationMap['props'] = props;
            transaction.update(userRef, {'roomDecoration': decorationMap});
          }
        }

        final memoRef = _firestore
            .collection('users')
            .doc(targetUserId)
            .collection('memos')
            .doc(noteId);
        transaction.delete(memoRef);

        final reportRef = _firestore.collection('reports').doc(reportId);
        transaction.update(reportRef, {'status': 'resolved'});

        final reporterNotiRef = _firestore.collection('notifications').doc();
        transaction.set(reporterNotiRef, {
          'userId': reporterId,
          'type': 'reportResult',
          'message': '신고하신 메모가 삭제 조치되었습니다. 건전한 커뮤니티를 위해 힘써주셔서 감사합니다.',
          'isRead': false,
          'createdAt': FieldValue.serverTimestamp(),
          'data': {'status': 'resolved'},
        });

        final violatorNotiRef = _firestore.collection('notifications').doc();
        transaction.set(violatorNotiRef, {
          'userId': targetUserId,
          'type': 'system',
          'message': '작성하신 메모가 신고 접수되어 커뮤니티 가이드라인 위반으로 삭제되었습니다.',
          'isRead': false,
          'createdAt': FieldValue.serverTimestamp(),
          'data': {'systemType': 'violator'},
        });
      });
      await fetchReports();
    } catch (e) {
      debugPrint('노트 삭제 오류: $e');
    }

    _isLoading = false;
    notifyListeners();
  }

  Future<void> rejectReport(
      String reportId, String reporterId, String rejectReason) async {
    _isLoading = true;
    notifyListeners();

    try {
      await _firestore.collection('notifications').add({
        'userId': reporterId,
        'type': 'reportResult',
        'message': '신고가 반려되었습니다. 사유: $rejectReason',
        'isRead': false,
        'createdAt': FieldValue.serverTimestamp(),
        'data': {'status': 'rejected', 'reason': rejectReason},
      });

      await _firestore.collection('reports').doc(reportId).update({
        'status': 'rejected',
        'rejectReason': rejectReason,
      });

      await fetchReports();
    } catch (e) {
      debugPrint('신고 반려 오류: $e');
    }

    _isLoading = false;
    notifyListeners();
  }

  Future<void> suspendUser({
    required String reportId,
    required String targetUserId,
    required String reporterId,
    required int days,
    String? reason,
  }) async {
    _isLoading = true;
    notifyListeners();

    try {
      await _firestore.runTransaction((transaction) async {
        final userRef = _firestore.collection('users').doc(targetUserId);
        final reporterRef = _firestore.collection('users').doc(reporterId);
        final reportRef = _firestore.collection('reports').doc(reportId);

        // 1. 모든 읽기(Read) 작업은 처음에 수행해야 함
        final reporterSnapshot = await transaction.get(reporterRef);

        // 2. 데이터 계산
        DateTime suspendedUntil;
        String durationStr;

        if (days == -1) {
          suspendedUntil = DateTime(2099, 12, 31); // Permanent
          durationStr = '영구';
        } else {
          suspendedUntil = DateTime.now().add(Duration(days: days));
          durationStr = '${days}일';
        }

        // 3. 쓰기(Write) 작업 수행
        transaction.update(userRef, {
          'suspendedUntil': Timestamp.fromDate(suspendedUntil),
          'suspensionReason': reason ?? '커뮤니티 가이드라인 위반',
        });

        transaction.update(reportRef, {'status': 'resolved'});

        final reporterNotiRef = _firestore.collection('notifications').doc();
        transaction.set(reporterNotiRef, {
          'userId': reporterId,
          'type': 'reportResult',
          'message':
              '신고하신 회원이 $durationStr 정지 처리되었습니다. 보상으로 100가지가 지급되었습니다. 건전한 커뮤니티를 위해 힘써주셔서 감사합니다.',
          'isRead': false,
          'createdAt': FieldValue.serverTimestamp(),
          'data': {'status': 'suspended', 'duration': durationStr},
        });

        // 신고자 보상 (100가지)
        if (reporterSnapshot.exists) {
          final currentPoints = reporterSnapshot.data()?['points'] ?? 0;
          transaction.update(reporterRef, {'points': currentPoints + 100});
        }
      });
      await fetchReports();
    } catch (e) {
      debugPrint('사용자 정지 오류: $e');
    }

    _isLoading = false;
    notifyListeners();
  }

  // 사용자 관리 기능
  List<UserModel> _allUsers = [];
  List<UserModel> get allUsers => _allUsers;
  String? _searchQuery;
  String? get searchQuery => _searchQuery;
  DocumentSnapshot? _lastUserDoc;
  bool _hasMoreUsers = true;
  bool get hasMoreUsers => _hasMoreUsers;

  Future<void> fetchUsers({bool isRefresh = false, String? searchQuery}) async {
    if (_isLoading) return;
    if (!isRefresh && !_hasMoreUsers && searchQuery == _searchQuery) return;

    // 검색어 변경 시 초기화
    if (searchQuery != _searchQuery) {
      isRefresh = true;
      _searchQuery = searchQuery;
    }

    _isLoading = true;
    notifyListeners();

    try {
      Query query = _firestore.collection('users');

      if (_searchQuery != null && _searchQuery!.isNotEmpty) {
        // 닉네임 전방 일치 검색
        query = query
            .where('nickname', isGreaterThanOrEqualTo: _searchQuery)
            .where('nickname', isLessThanOrEqualTo: '$_searchQuery\uf8ff')
            .orderBy('nickname');
      } else {
        query = query.orderBy('createdAt', descending: true);
      }

      query = query.limit(20);

      if (!isRefresh && _lastUserDoc != null) {
        query = query.startAfterDocument(_lastUserDoc!);
      }

      final snapshot = await query.get();

      if (isRefresh) {
        _allUsers = [];
        _lastUserDoc = null;
        _hasMoreUsers = true;
      }

      if (snapshot.docs.isNotEmpty) {
        final newUsers =
            snapshot.docs.map((doc) => UserModel.fromFirestore(doc)).toList();
        _allUsers.addAll(newUsers);
        _lastUserDoc = snapshot.docs.last;
        _hasMoreUsers = snapshot.docs.length == 20;
      } else {
        _hasMoreUsers = false;
      }
    } catch (e) {
      debugPrint('사용자 목록 가져오기 오류: $e');
    }

    _isLoading = false;
    notifyListeners();
  }

  Future<void> unsuspendUser(String userId) async {
    try {
      _isLoading = true;
      notifyListeners();

      await _firestore.collection('users').doc(userId).update({
        'suspendedUntil': FieldValue.delete(),
        'suspensionReason': FieldValue.delete(),
      });

      _isLoading =
          false; // fetchUsers를 호출하기 전에 꺼주어서 fetchUsers 내부의 중복 실행 방지 로직을 통과하게 함
      await fetchUsers(isRefresh: true, searchQuery: _searchQuery);
    } catch (e) {
      debugPrint('정지 해제 오류: $e');
      _isLoading = false;
      notifyListeners();
    }
  }

  DateTime _selectedDate = DateTime.now();
  DateTime get selectedDate => _selectedDate;

  void setSelectedDate(DateTime date) {
    _selectedDate = date;
    fetchStats();
  }

  Future<void> updateUserPoints(String userId, int points) async {
    try {
      _isLoading = true;
      notifyListeners();

      await _firestore.collection('users').doc(userId).update({
        'points': points,
      });

      _isLoading = false;
      await fetchUsers(isRefresh: true, searchQuery: _searchQuery);
    } catch (e) {
      debugPrint('가지 수정 오류: $e');
      _isLoading = false;
      notifyListeners();
    }
  }

  int _dailyVisitorCount = 0;
  int get dailyVisitorCount => _dailyVisitorCount;

  int _totalUserCount = 0;
  int get totalUserCount => _totalUserCount;

  int _todayNewUserCount = 0;
  int get todayNewUserCount => _todayNewUserCount;

  int _todayDiaryCount = 0;
  int get todayDiaryCount => _todayDiaryCount;

  int _todayAdViewerCount = 0;
  int get todayAdViewerCount => _todayAdViewerCount;

  int _todayAdImpressionCount = 0;
  int get todayAdImpressionCount => _todayAdImpressionCount;
  
  int _iosUserCount = 0;
  int get iosUserCount => _iosUserCount;
  
  int _androidUserCount = 0;
  int get androidUserCount => _androidUserCount;

  Map<int, int> _hourlyLoginStats = {};
  Map<int, int> get hourlyLoginStats => _hourlyLoginStats;

  Map<String, int> _countryStats = {};
  Map<String, int> get countryStats => _countryStats;

  Future<void> fetchStats() async {
    if (_isDisposed) return;
    try {
      final startOfToday = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day);
      final endOfToday = startOfToday.add(const Duration(days: 1)).subtract(const Duration(milliseconds: 1));

      // 오늘 접속자 (login_history + lastLoginDate 병합하여 과도기 버전 대응)
      // 1. login_history에서 UID 추출
      final historySnap = await _firestore
          .collection('login_history')
          .where('loginDate',
              isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
          .where('loginDate',
              isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
          .get();
      final historyUids = historySnap.docs.map((d) => d['userId'] as String).toSet();

      // 2. users 컬렉션의 lastLoginDate에서 UID 추출 (업데이트 전 사용자 대응)
      final legacySnap = await _firestore
          .collection('users')
          .where('lastLoginDate',
              isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
          .where('lastLoginDate',
              isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
          .get();
      final legacyUids = legacySnap.docs.map((doc) => doc.id).toSet();
      
      // 3. 중복 제거하여 합치기
      _dailyVisitorCount = historyUids.union(legacyUids).length;
      
      // 시간대별 접속자 통계 (읽기 비용 문제로 프론트엔드 직접 계산 제거, 추후 백엔드 어그리게이션 필요)
      _hourlyLoginStats = {};

      // 신규 가입자
      final newUsersQuery = await _firestore
          .collection('users')
          .where('createdAt',
              isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
          .where('createdAt',
              isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
          .count()
          .get();
      _todayNewUserCount = newUsersQuery.count ?? 0;

      // 오늘 생성된 일기 수
      final todayDiaryQuery = await _firestore
          .collection('diaries')
          .where('createdAt',
              isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
          .where('createdAt',
              isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
          .count()
          .get();
      _todayDiaryCount = todayDiaryQuery.count ?? 0;

      // 복합 인덱스 오류를 피하기 위해 날짜로만 쿼리 후 로컬 필터링
      final adLogsSnap = await _firestore
          .collection('ad_logs')
          .where('timestamp',
              isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
          .where('timestamp',
              isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
          .get();
          
      final showDocs = adLogsSnap.docs.where((doc) {
        final data = doc.data();
        return data.containsKey('action') && data['action'] == 'show';
      }).toList();

      _todayAdImpressionCount = showDocs.length;
      final uniqueViewerIds = showDocs.map((d) => d['userId'] as String).toSet();
      _todayAdViewerCount = uniqueViewerIds.length;

      final totalQuery = await _firestore.collection('users').count().get();
      _totalUserCount = totalQuery.count ?? 0;

      // 전체 유저 대상 국가 통계 계산 (읽기 비용 발생하지만 요청에 따라 구현)
      final allUsers = await _firestore.collection('users').get();
      _countryStats = {};
      for (var doc in allUsers.docs) {
        final country = doc.data()['countryCode'] as String? ?? '알수없음';
        _countryStats[country] = (_countryStats[country] ?? 0) + 1;
      }
      
      // 플랫폼별 사용자 수
      final iosQuery = await _firestore
          .collection('users')
          .where('platform', isEqualTo: 'ios')
          .count()
          .get();
      _iosUserCount = iosQuery.count ?? 0;
      
      final androidQuery = await _firestore
          .collection('users')
          .where('platform', isEqualTo: 'android')
          .count()
          .get();
      _androidUserCount = androidQuery.count ?? 0;
    } catch (e) {
      debugPrint('통계 불러오기 오류: $e');
    }
    if (_isDisposed) return;
    notifyListeners();
  }

  // 대시보드 팝업용 상세 리스트 조회
  Future<List<UserModel>> getTodayNewUsers() async {
    final startOfToday = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day);
    final endOfToday = startOfToday.add(const Duration(days: 1)).subtract(const Duration(milliseconds: 1));
    final snap = await _firestore
        .collection('users')
        .where('createdAt',
            isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
        .where('createdAt',
            isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
        .get();
    return snap.docs.map((d) => UserModel.fromFirestore(d)).toList();
  }

  Future<List<UserModel>> getTodayLoginUsers() async {
    final startOfToday = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day);
    final endOfToday = startOfToday.add(const Duration(days: 1)).subtract(const Duration(milliseconds: 1));
    
    // 1. login_history 조회
    final historySnap = await _firestore
        .collection('login_history')
        .where('loginDate',
            isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
        .where('loginDate',
            isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
        .get();
    final historyUids = historySnap.docs.map((d) => d['userId'] as String).toSet();

    // 2. users.lastLoginDate 조회
    final legacySnap = await _firestore
        .collection('users')
        .where('lastLoginDate',
            isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
        .where('lastLoginDate',
            isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
        .get();
    final legacyUids = legacySnap.docs.map((doc) => doc.id).toSet();
        
    final allUids = historyUids.union(legacyUids).toList();
    if (allUids.isEmpty) return [];

    // 유저 상세 정보 가져오기 (30개씩 분할 처리)
    List<UserModel> users = [];
    for (var i = 0; i < allUids.length; i += 30) {
      final chunk = allUids.sublist(i, 
          i + 30 > allUids.length ? allUids.length : i + 30);
      final userSnap = await _firestore
          .collection('users')
          .where(FieldPath.documentId, whereIn: chunk)
          .get();
      users.addAll(userSnap.docs.map((d) => UserModel.fromFirestore(d)));
    }
    
    users.sort((a, b) {
      if (a.lastLoginDate == null && b.lastLoginDate == null) return 0;
      if (a.lastLoginDate == null) return 1;
      if (b.lastLoginDate == null) return -1;
      return a.lastLoginDate!.compareTo(b.lastLoginDate!);
    });
    return users;
  }

  Future<List<UserModel>> getTodayAdViewerUsers() async {
    final startOfToday = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day);
    final endOfToday = startOfToday.add(const Duration(days: 1)).subtract(const Duration(milliseconds: 1));
    
    // 1. ad_logs에서 오늘 로그 조회
    final logSnap = await _firestore
        .collection('ad_logs')
        .where('timestamp',
            isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
        .where('timestamp',
            isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
        .get();
        
    final showDocs = logSnap.docs.where((doc) {
      final data = doc.data();
      return data.containsKey('action') && data['action'] == 'show';
    }).toList();
    
    // 2. 유니크 유저 ID 추출 및 마지막 시청 시간 맵핑
    final uids = showDocs.map((d) => d['userId'] as String).toSet().toList();
    if (uids.isEmpty) return [];

    final lastShowTimeMap = <String, DateTime>{};
    for (var doc in showDocs) {
      final uid = doc['userId'] as String;
      final time = (doc['timestamp'] as Timestamp).toDate();
      if (lastShowTimeMap[uid] == null || time.isAfter(lastShowTimeMap[uid]!)) {
        lastShowTimeMap[uid] = time;
      }
    }

    // 3. 해당 유저들의 상세 정보 조회 (30개씩 분할)
    List<UserModel> users = [];
    for (var i = 0; i < uids.length; i += 30) {
      final chunk = uids.sublist(i, 
          i + 30 > uids.length ? uids.length : i + 30);
      final userSnap = await _firestore
          .collection('users')
          .where(FieldPath.documentId, whereIn: chunk)
          .get();
      
      for (var doc in userSnap.docs) {
        var user = UserModel.fromFirestore(doc);
        // 리스트 표시용으로 마지막 광고 시간을 lastAdRewardDate에 임시 주입 (UI 호환성)
        if (lastShowTimeMap.containsKey(user.uid)) {
          user = user.copyWith(lastAdRewardDate: lastShowTimeMap[user.uid]);
        }
        users.add(user);
      }
    }
    
    // 4. 최신 시청순 정렬
    users.sort((a, b) {
      if (a.lastAdRewardDate == null && b.lastAdRewardDate == null) return 0;
      if (a.lastAdRewardDate == null) return 1;
      if (b.lastAdRewardDate == null) return -1;
      return b.lastAdRewardDate!.compareTo(a.lastAdRewardDate!);
    });
    
    return users;
  }

  Future<List<Map<String, dynamic>>> getTodayAdImpressions() async {
    final startOfToday = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day);
    final endOfToday = startOfToday.add(const Duration(days: 1)).subtract(const Duration(milliseconds: 1));
    
    final logSnap = await _firestore
        .collection('ad_logs')
        .where('timestamp',
            isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
        .where('timestamp',
            isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
        .get();
        
    final showDocs = logSnap.docs.where((doc) {
      final data = doc.data();
      return data.containsKey('action') && data['action'] == 'show';
    }).toList();
    
    final viewCountMap = <String, int>{};
    final lastShowTimeMap = <String, DateTime>{};
    
    for (var doc in showDocs) {
      final uid = doc['userId'] as String;
      final time = (doc['timestamp'] as Timestamp).toDate();
      viewCountMap[uid] = (viewCountMap[uid] ?? 0) + 1;
      if (lastShowTimeMap[uid] == null || time.isAfter(lastShowTimeMap[uid]!)) {
        lastShowTimeMap[uid] = time;
      }
    }
    
    final uids = viewCountMap.keys.toList();
    if (uids.isEmpty) return [];

    List<Map<String, dynamic>> result = [];
    for (var i = 0; i < uids.length; i += 30) {
      final chunk = uids.sublist(i, i + 30 > uids.length ? uids.length : i + 30);
      final userSnap = await _firestore
          .collection('users')
          .where(FieldPath.documentId, whereIn: chunk)
          .get();
      
      for (var doc in userSnap.docs) {
        result.add({
          'user': UserModel.fromFirestore(doc),
          'count': viewCountMap[doc.id] ?? 0,
          'lastViewTime': lastShowTimeMap[doc.id] ?? DateTime.now(),
        });
      }
    }
    
    result.sort((a, b) {
      final countA = a['count'] as int;
      final countB = b['count'] as int;
      if (countB != countA) {
        return countB.compareTo(countA); // 횟수 내림차순 정렬
      }
      final dateA = a['lastViewTime'] as DateTime;
      final dateB = b['lastViewTime'] as DateTime;
      return dateB.compareTo(dateA); // 최근 시청순 정렬
    });
    
    return result;
  }

  Future<List<Map<String, dynamic>>> getTodayDiaries() async {
    final startOfToday = DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day);
    final endOfToday = startOfToday.add(const Duration(days: 1)).subtract(const Duration(milliseconds: 1));
    final snap = await _firestore
        .collection('diaries')
        .where('createdAt',
            isGreaterThanOrEqualTo: Timestamp.fromDate(startOfToday))
        .where('createdAt',
            isLessThanOrEqualTo: Timestamp.fromDate(endOfToday))
        .get();

    final diaries = snap.docs.map((d) => {...d.data(), 'id': d.id}).toList();

    // 작성자의 닉네임 가져오기 (중복 제거 후 일괄 조회하여 쿼리 최적화)
    final userIds = diaries
        .map((d) => d['userId'] as String?)
        .whereType<String>()
        .toSet()
        .toList();
    final nicknameMap = <String, String>{};
    final platformMap = <String, String?>{};

    // Firestore whereIn은 최대 30개씩 지원
    for (var i = 0; i < userIds.length; i += 30) {
      final chunk =
          userIds.sublist(i, i + 30 > userIds.length ? userIds.length : i + 30);
      final userSnap = await _firestore
          .collection('users')
          .where(FieldPath.documentId, whereIn: chunk)
          .get();
      for (var doc in userSnap.docs) {
        nicknameMap[doc.id] = doc.data()['nickname'] ?? '알 수 없음';
        platformMap[doc.id] = doc.data()['platform'];
      }
    }

    for (var diary in diaries) {
      final userId = diary['userId'] as String?;
      diary['nickname'] = nicknameMap[userId] ?? '알 수 없음';
      diary['platform'] = platformMap[userId];
    }

    return diaries;
  }

  // --- 버전 관리 (Version Management) ---
  VersionModel? _androidVersionInfo;
  VersionModel? get androidVersionInfo => _androidVersionInfo;

  VersionModel? _iosVersionInfo;
  VersionModel? get iosVersionInfo => _iosVersionInfo;

  // 기존 호환성 유지용 (필요시)
  VersionModel? get versionInfo => _androidVersionInfo;

  Future<void> fetchVersionInfo() async {
    try {
      final doc = await _firestore.collection('settings').doc('version').get();
      final data = doc.data();
      if (doc.exists && data is Map<String, dynamic>) {
        // 안드로이드 정보
        if (data.containsKey('android')) {
          _androidVersionInfo = VersionModel.fromMap(data['android']);
        } else {
          // 레거시: 루트에 있는 정보를 안드로이드로 간주
          _androidVersionInfo = VersionModel.fromFirestore(doc);
        }

        // iOS 정보
        if (data.containsKey('ios')) {
          _iosVersionInfo = VersionModel.fromMap(data['ios']);
        } else {
          // 레거시: 루트에 있는 정보를 우선 iOS로도 사용 (나중에 분리 저장 시점부터 갈라짐)
          _iosVersionInfo = VersionModel.fromFirestore(doc);
        }
      } else {
        // 기본값 설정
        final defaultVersion = VersionModel(
          latestVersion: '1.0.0',
          minimumVersion: '1.0.0',
          updateTitle: '업데이트 안내',
          updateBody: '새로운 버전이 출시되었습니다. 업데이트 후 이용해 주세요!',
          updateTitleEn: 'Update Notice',
          updateBodyEn: 'A new version is available. Please update to the latest version for a better experience!',
          updateTitleJa: 'アップデートのお知らせ',
          updateBodyJa: '新しいバージョンがリリースされました。より安定したサービスのためにアップデートをお願いします！',
          isForceUpdate: false,
        );
        _androidVersionInfo = defaultVersion;
        _iosVersionInfo = defaultVersion;
      }
    } catch (e) {
      debugPrint('버전 정보 불러오기 오류: $e');
    }
    notifyListeners();
  }

  Future<void> updateVersionInfo(VersionModel version, String platform) async {
    _isLoading = true;
    notifyListeners();
    try {
      final Map<String, dynamic> updateData = {
        platform: version.toFirestore(),
      };

      // 안드로이드 업데이트 시 레거시 앱 호환을 위해 루트 필드도 업데이트
      if (platform == 'android') {
        updateData.addAll(version.toFirestore());
      }

      await _firestore
          .collection('settings')
          .doc('version')
          .set(updateData, SetOptions(merge: true));
      
      if (platform == 'android') {
        _androidVersionInfo = version;
      } else {
        _iosVersionInfo = version;
      }
    } catch (e) {
      debugPrint('버전 정보 업데이트 오류 ($platform): $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  // --- 공지사항 관리 (Notice) ---
  List<Map<String, dynamic>> _notices = [];
  List<Map<String, dynamic>> get notices => _notices;

  Future<void> fetchNotices() async {
    if (_isDisposed) return;
    _isLoading = true;
    notifyListeners();
    try {
      final snap = await _firestore
          .collection('notices')
          .orderBy('createdAt', descending: true)
          .get();
      _notices = snap.docs.map((d) => {...d.data(), 'id': d.id}).toList();
    } catch (e) {
      debugPrint('공지사항 불러오기 오류: $e');
    }
    if (_isDisposed) return;
    _isLoading = false;
    notifyListeners();
  }

  Future<void> createNotice({
    required String title,
    required String content,
    required String category,
    required bool isPinned,
    DateTime? publishAt,
  }) async {
    _isLoading = true;
    notifyListeners();
    try {
      await _firestore.collection('notices').add({
        'title': title,
        'content': content,
        'category': category,
        'isPinned': isPinned,
        'publishAt': publishAt != null ? Timestamp.fromDate(publishAt) : null,
        'createdAt': FieldValue.serverTimestamp(),
      });
      await fetchNotices();
    } catch (e) {
      debugPrint('공지사항 작성 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  Future<void> deleteNotice(String id) async {
    _isLoading = true;
    notifyListeners();
    try {
      await _firestore.collection('notices').doc(id).delete();
      await fetchNotices();
    } catch (e) {
      debugPrint('공지사항 삭제 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  // --- 푸시 메시지 관리 (Push) ---
  List<Map<String, dynamic>> _pushHistory = [];
  List<Map<String, dynamic>> get pushHistory => _pushHistory;

  Future<void> fetchPushHistory() async {
    if (_isDisposed) return;
    _isLoading = true;
    notifyListeners();
    try {
      final snap = await _firestore
          .collection('push_history')
          .orderBy('sentAt', descending: true)
          .get();
      _pushHistory = snap.docs.map((d) => {...d.data(), 'id': d.id}).toList();
    } catch (e) {
      debugPrint('푸시 기록 불러오기 오류: $e');
    }
    if (_isDisposed) return;
    _isLoading = false;
    notifyListeners();
  }

  Future<void> sendPushMessage({
    required String title,
    required String body,
    required String target,
    String? deepLink,
  }) async {
    _isLoading = true;
    notifyListeners();
    try {
      // FCM 전송 로직 (추후 Cloud Function과 연동)
      // 현재는 기록용 저장
      await _firestore.collection('push_history').add({
        'title': title,
        'body': body,
        'target': target,
        'deepLink': deepLink,
        'sentAt': FieldValue.serverTimestamp(),
        'status': 'sent',
      });
      await fetchPushHistory();
    } catch (e) {
      debugPrint('푸시 발송 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
  }

  // --- 광고 로그 관리 (Ad Logs) ---
  List<AdLogModel> _adLogs = [];
  List<AdLogModel> get adLogs => _adLogs;

  DocumentSnapshot? _lastAdLogDoc;
  bool _hasMoreAdLogs = true;
  bool get hasMoreAdLogs => _hasMoreAdLogs;
  bool _isLoadingMoreAdLogs = false;
  bool get isLoadingMoreAdLogs => _isLoadingMoreAdLogs;

  Future<void> fetchAdLogs({int limit = 20, bool refresh = true}) async {
    if (_isDisposed) return;

    if (refresh) {
      _adLogs = [];
      _lastAdLogDoc = null;
      _hasMoreAdLogs = true;
      _isLoading = true;
    } else {
      if (!_hasMoreAdLogs || _isLoadingMoreAdLogs) return;
      _isLoadingMoreAdLogs = true;
    }

    notifyListeners();

    try {
      final snapshot = await _adService.getAdLogsPaginated(
        lastDocument: _lastAdLogDoc,
        limit: limit,
      );

      final newLogs =
          snapshot.docs.map((doc) => AdLogModel.fromFirestore(doc)).toList();

      if (refresh) {
        _adLogs = newLogs;
      } else {
        _adLogs.addAll(newLogs);
      }

      if (snapshot.docs.isNotEmpty) {
        _lastAdLogDoc = snapshot.docs.last;
      }

      _hasMoreAdLogs = newLogs.length == limit;
    } catch (e) {
      debugPrint('광고 로그 불러오기 오류: $e');
    }

    if (_isDisposed) return;
    _isLoading = false;
    _isLoadingMoreAdLogs = false;
    notifyListeners();
  }

  Future<void> loadMoreAdLogs() async {
    await fetchAdLogs(refresh: false);
  }

  Future<UserModel?> findUserByTarget(String target) async {
    try {
      if (target.contains('@')) {
        // 이메일로 검색 (대소문자 구분 없이 검색하기 위해 소문자 변환 시도 고려 가능하나 DB 값이 소문자여야 함)
        // 일단 정확히 일치하는 경우 검색
        final snap = await _firestore
            .collection('users')
            .where('email', isEqualTo: target)
            .limit(1)
            .get();
        if (snap.docs.isNotEmpty) {
          return UserModel.fromFirestore(snap.docs.first);
        }

        // 혹시 모르니 소문자로도 한번 더 시도
        final snapLower = await _firestore
            .collection('users')
            .where('email', isEqualTo: target.toLowerCase())
            .limit(1)
            .get();
        if (snapLower.docs.isNotEmpty) {
          return UserModel.fromFirestore(snapLower.docs.first);
        }
      } else {
        // UID로 검색
        final doc = await _firestore.collection('users').doc(target).get();
        if (doc.exists) {
          return UserModel.fromFirestore(doc);
        }
      }
    } catch (e) {
      debugPrint('유저 찾기 오류: $e');
    }
    return null;
  }

  // --- 유저 로그 확인 (Timeline) ---
  List<Map<String, dynamic>> _userLogs = [];
  List<Map<String, dynamic>> get userLogs => _userLogs;

  Future<void> fetchUserLogs(String uid) async {
    if (_isDisposed) return;
    _isLoading = true;
    notifyListeners();
    try {
      final snap = await _firestore
          .collection('notifications')
          .where('userId', isEqualTo: uid)
          .orderBy('createdAt', descending: true)
          .limit(50)
          .get();
      _userLogs =
          snap.docs.map((doc) => {...doc.data(), 'id': doc.id}).toList();
    } catch (e) {
      debugPrint('유저 로그 불러오기 오류: $e');
      _userLogs = [];
    }
    if (_isDisposed) return;
    _isLoading = false;
    notifyListeners();
  }

  // 전체 번역 맵 (100개+ 마스터 데이터)
  static const Map<String, Map<String, String>> questionTranslationMap = {
    '지금 내 주변에서 가장 아름답게 느껴지는 사물이나 소리는?': {
      'en': 'What objects or sounds around you feel the most beautiful right now?',
      'ja': '今私の周りで最も美しく感じるものや音は？',
    },
    '최근에 읽은 책이나 본 영상 중 기억에 남는 문구는?': {
      'en': 'What is a memorable quote from a book you read or a video you watched recently?',
      'ja': '最近読んだ本や見た映像の中で記憶に残るフレーズは？',
    },
    '오늘 나에게 가장 필요한 위로 또는 격려는 무엇인가요?': {
      'en': 'What comfort or encouragement do you need most today?',
      'ja': '今日私にとって最も必要な慰めや励ましは何ですか？',
    },
    '나의 건강을 위해 오늘 할 수 있는 작은 행동 하나는?': {
      'en': 'What is one small action you can do for your health today?',
      'ja': '私の健康のために今日することができる小さい行動1つはか。',
    },
    '오늘 하루가 지나기 전에 꼭 연락하고 싶은 사람이 있나요?': {
      'en': 'Is there anyone you definitely want to contact before today is over?',
      'ja': '今日一日が過ぎる前に必ず連絡したい人はいますか？',
    },
    '어제 꿈속에서 혹은 잠결에 느꼈던 특별한 감정이 있나요?': {
      'en': 'Was there any special feeling you had in your dream or while sleeping yesterday?',
      'ja': '昨日の夢の中で、あるいは暫定に感じた特別な感情はありますか？',
    },
    '지금 내 기분을 색깔로 표현한다면 어떤 색일까요?': {
      'en': 'If you were to express your mood right now in a color, what color would it be?',
      'ja': '今私の気分を色で表現したらどういう色でしょうか？',
    },
    '내가 생각하는 \'행복한 하루\'의 정의는 무엇인가요?': {
      'en': 'What is your definition of a \'happy day\'?',
      'ja': '私が考える「幸せな一日」の定義は何ですか？',
    },
    '오늘 하루를 무사히 마칠 수 있음에 대해 미리 감사해본다면?': {
      'en': 'If you were to give thanks in advance for being able to finish today safely?',
      'ja': '今日一日を無事に終えることができることを事前に感謝していますか？',
    },
    '내가 좋아하는 향기나 냄새를 오늘 떠올려 본다면?': {
      'en': 'If you were to think of a scent or smell you like today?',
      'ja': '私が好きな香りや香りを今日思い出してみると？',
    },
    '오늘 하루 중 나만의 온전한 휴식 시간은 몇 시인가요?': {
      'en': 'What time today do you have for your own complete rest?',
      'ja': '今日の一日のうちに自分の完全な休憩時間は何時ですか？',
    },
    '어제의 나보다 오늘 조금 더 발전하고 싶은 부분은?': {
      'en': 'What part of yourself do you want to develop a bit more today than yesterday?',
      'ja': '昨日の私より今日もう少し発展したいところは？',
    },
    '내가 존경하는 사람이 오늘 나의 모습이라면 어떻게 행동할까요?': {
      'en': 'If the person I respect were in my shoes today, how would they act?',
      'ja': '私が尊敬する人が今日の私の姿であれば、どのように行動しますか？',
    },
    '내 주변 환경에서 오늘 소소한 변화를 주고 싶은 것은?': {
      'en': 'What small change would I like to make to my surroundings today?',
      'ja': '私の周りの環境で今日のマイナーな変化を与えたいですか？',
    },
    '오늘 하루가 끝나고 침대에 누웠을 때 어떤 기분을 느끼고 싶나요?': {
      'en': 'What kind of feeling do you want to have when you lie in bed at the end of the day?',
      'ja': '今日は一日の終わりにベッドに横たわったときにどのような気分になりたいですか？',
    },
    '오늘 마실 차나 커피 한 잔에 담고 싶은 마음은?': {
      'en': 'What kind of heart do you want to put into a cup of tea or coffee you drink today?',
      'ja': '今日飲んだお茶やコーヒー1杯に入れたい気持ちは？',
    },
    '지금 내가 가장 간절히 바라는 소망 하나는 무엇인가요?': {
      'en': 'What is the one wish that I most earnestly desire right now?',
      'ja': '今私が一番切実に望む一つは何ですか？',
    },
    '오늘 저녁 식사로 무엇을 먹으며 행복을 느끼고 싶나요?': {
      'en': 'What do you want to eat for dinner today and feel happy?',
      'ja': '今日の夕食で何を食べて幸せを感じたいですか？',
    },
    '오늘 하루 동안 나의 에너지를 가장 많이 쓰고 싶은 곳은?': {
      'en': 'Where do you want to spend the most of your energy throughout the day today?',
      'ja': '今日の一日の中で私のエネルギーを一番使いたいところは？',
    },
    '오늘 하루 중 가장 평화로울 것 같은 시간은 언제인가요?': {
      'en': 'When do you think will be the most peaceful time of your day today?',
      'ja': '今日の一日で最も平和なような時間はいつですか？',
    },
    '오늘 나를 힘들게 할 것 같은 상황이 있다면 어떻게 대처할까요?': {
      'en': 'How will I handle any situations that might make today difficult for me?',
      'ja': '今日、私を苦労しそうな状況があればどう対処するのでしょうか？',
    },
    '내가 가장 좋아하는 계절의 분위기를 오늘 어떻게 느낄 수 있을까요?': {
      'en': 'How can I feel the atmosphere of my favorite season today?',
      'ja': '私が好きな季節の雰囲気を今日どのように感じることができますか？',
    },
    '내가 요즘 가장 몰입하고 있는 관심사는 무엇인가요?': {
      'en': 'What is the interest that I am most immersed in these days?',
      'ja': '私が最近最も没入している関心事は何ですか？',
    },
    '오늘 나의 패션이나 소품 중 가장 마음에 드는 포인트는?': {
      'en': 'What is my favorite point of my fashion or accessories today?',
      'ja': '今日私のファッションや小道具の中で一番好きなポイントは？',
    },
    '오늘 가장 완벽한 순간을 상상해본다면 어떤 모습일까요?': {
      'en': 'What would my most perfect moment today look like if I were to imagine it?',
      'ja': '今日一番完璧な瞬間を想像してみたらどうなるのでしょうか？',
    },
    '오늘 하루를 영화로 만든다면 어떤 장르였으면 좋겠나요?': {
      'en': 'If today were made into a movie, what genre would I want it to be?',
      'ja': '今日一日を映画にしたら、どんなジャンルだったらいいですか？',
    },
    '어제 해결하지 못한 걱정이 있다면 어떻게 긍정적으로 바라볼까요?': {
      'en': 'How can I look positively at any worries I couldn\'t resolve yesterday?',
      'ja': '昨日解決できなかった心配があれば、どうやって肯定的に見てみましょうか？',
    },
    '오늘 꼭 완수하고 싶은 가장 중요한 일 하나는?': {
      'en': 'What is the one most important thing I definitely want to complete today?',
      'ja': '今日必ず果たしたい最も重要なことの一つは？',
    },
    '오늘 하루 동안 내가 집중하고 싶은 단어 하나를 정한다면?': {
      'en': 'If I were to choose one word to focus on throughout the day today?',
      'ja': '今日の日中に私が集中したいと思う言葉を決めたら？',
    },
    '오늘 하루 동안 내가 도전해보고 싶은 아주 사소한 일은?': {
      'en': 'What is a very small thing I want to challenge myself with today?',
      'ja': '今日の日に私が挑戦したいと思う非常に些細なことは？',
    },
    '오늘 누군가를 위해 할 수 있는 아주 작은 친절은?': {
      'en': 'What is a small act of kindness you can do for someone today?',
      'ja': '今日誰かのためにできる非常に小さい親切はか。',
    },
    '어제 나를 가장 기쁘게 했던 일은 무엇인가요?': {
      'en': 'What made you the happiest yesterday?',
      'ja': '昨日私を最も喜ばせたことは何ですか？',
    },
    '어제보다 오늘 더 향상시키고 싶은 나의 성격은?': {
      'en': 'What part of your personality do you want to improve today more than yesterday?',
      'ja': '昨日より今日より向上させたい私の性格は？',
    },
    '오늘 나에게 주는 작은 선물로 무엇을 선택하고 싶나요?': {
      'en': 'What small gift would you like to give yourself today?',
      'ja': '今日私に与える小さな贈り物として何を選びたいですか？',
    },
    '오늘 내가 만날 사람들 중 가장 반가운 사람은 누구인가요?': {
      'en': 'Who is the person you are most excited to meet today?',
      'ja': '今日私が出会う人の中で最も嬉しい人は誰ですか？',
    },
    '지금 창밖의 풍경에서 발견할 수 있는 긍정적인 요소는?': {
      'en': 'What is a positive element you can find in the view outside the window right now?',
      'ja': '今、窓の外の風景で発見できる肯定的な要素は？',
    },
    '나의 목표에 한 걸음 더 다가가기 위해 오늘 할 일은?': {
      'en': 'What will you do today to get one step closer to your goal?',
      'ja': '私の目標に一歩近づくために今日することは？',
    },
    '세상에서 내가 가장 편안함을 느끼는 장소는 어디인가요?': {
      'en': 'Where is the place where you feel the most comfortable in the world?',
      'ja': '世界で私が最も快適に感じる場所はどこですか？',
    },
    '내가 오늘 새롭게 배워보고 싶은 작은 지식이나 정보가 있나요?': {
      'en': 'Is there any small piece of knowledge or information you want to learn today?',
      'ja': '私は今日新しく学びたいと思う小さな知識や情報がありますか？',
    },
    '누군가에게 오늘 전하고 싶은 따뜻한 감사의 말은?': {
      'en': 'What warm words of gratitude do you want to convey to someone today?',
      'ja': '誰かに今日伝えたい温かい感謝の言葉は？',
    },
    '오늘 아침 가장 먼저 떠오른 생각은 무엇인가요?': {
      'en': 'What was the very first thought that came to your mind this morning?',
      'ja': '今朝一番最初に思いついた考えは何ですか？',
    },
    '나를 둘러싼 소중한 사람들 중 오늘 더 사랑을 표현하고 싶은 사람은?': {
      'en': 'Of the precious people around you, who do you want to express more love to today?',
      'ja': '私を取り巻く大切な人の中で、今日もっと愛を表現したい人は？',
    },
    '내가 가진 재능 중 오늘 더 빛내고 싶은 것은?': {
      'en': 'Which of your talents do you want to shine more today?',
      'ja': '私が持っている才能の中で今日もっと輝かせたいのは？',
    },
    '오늘 하루 동안 내가 피하고 싶은 부정적인 생각은?': {
      'en': 'What negative thoughts do you want to avoid throughout the day today?',
      'ja': '今日の日に私が避けたいと思う否定的な考えは？',
    },
    '나의 소소한 습관 중 오늘 꼭 지키고 싶은 것은?': {
      'en': 'Which of your small habits do you definitely want to keep today?',
      'ja': '私のマイナーな習慣の中で今日必ず守りたいのは？',
    },
    '최근 나를 가장 크게 웃게 했던 일은 무엇인가요?': {
      'en': 'What was the thing that made you laugh the most recently?',
      'ja': '最近私を最も笑わせたことは何ですか？',
    },
    '오늘 하루를 시작하며 나에게 들려주고 싶은 노래는?': {
      'en': 'What song do you want to play for yourself as you start the day today?',
      'ja': '今日一日を始めて、私に聞きたい曲は？',
    },
    '내가 아끼는 물건이나 공간이 주는 의미는 무엇인가요?': {
      'en': 'What is the meaning of the object or space you cherish?',
      'ja': '私が大切なものや空間が与える意味は何ですか？',
    },
    '내가 어렸을 때 가졌던 꿈이 오늘 나의 삶에 어떤 영향을 주나요?': {
      'en': 'How does the dream you had when you were young affect your life today?',
      'ja': '私が幼い頃に持っていた夢は、今日の私の人生にどのような影響を与えますか？',
    },
    '어제보다 오늘 더 성장하고 싶은 나의 모습은?': {
      'en': 'In what way do you want to grow more today than yesterday?',
      'ja': '昨日より今日もっと成長したい私の姿は？',
    },
    '오늘 하루를 마무리하며 나에게 해주고 싶은 말은?': {
      'en': 'What do you want to say to yourself at the end of the day?',
      'ja': '今日一日を終えて私にやりたいことは？',
    },
    '내가 오늘 가장 집중하고 싶은 일은 무엇인가요?': {
      'en': 'What do you want to focus on the most today?',
      'ja': '私が今日最も集中したいことは何ですか？',
    },
    '오늘 나를 기쁘게 할 작은 계획이 있다면?': {
      'en': 'What is a small plan that will make you happy today?',
      'ja': '今日、私を喜ばせる小さな計画があれば？',
    },
    '오늘 내가 마주할 가장 큰 도전은 무엇일까요?': {
      'en': 'What will be the biggest challenge you face today?',
      'ja': '今日私が直面する最大の課題は何ですか？',
    },
    '나의 강점 중 오늘 가장 활용하고 싶은 부분은?': {
      'en': 'Which of your strengths do you want to use the most today?',
      'ja': '私の強みの中で今日最も活用したい部分は？',
    },
    '오늘 하루 중 내가 가장 기대하는 시간은?': {
      'en': 'What is the time of day you are most looking forward to today?',
      'ja': '今日の一日の中で一番期待している時間は？',
    },
    '내가 감사함을 느끼는 나의 신체 일부분은 어디인가요?': {
      'en': 'What part of your body are you grateful for?',
      'ja': '私が感謝している私の体の一部はどこですか？',
    },
    '오늘 내가 만난 풍경 중 가장 아름다운 것은?': {
      'en': 'What was the most beautiful scenery you encountered today?',
      'ja': '今日私が出会った風景の中で最も美しいのは？',
    },
    '나의 하루를 더 활기차게 만들어 줄 사소한 행동은?': {
      'en': 'What small action will make your day more energetic?',
      'ja': '私の一日をもっと活気にさせてくれる些細な行動は？',
    },
    '내 인생에서 가장 소중한 기억 중 하나를 떠올려 본다면?': {
      'en': 'If you could recall one of the most precious memories in your life?',
      'ja': '私の人生で最も大切な記憶の1つを思い出してみてください。',
    },
    '오늘 내가 나 자신에게 허락하고 싶은 휴식은?': {
      'en': 'What kind of rest do you want to allow yourself today?',
      'ja': '今日私が自分に許したい休憩は？',
    },
    '내가 오늘 하루 중 가장 많은 시간을 보낼 공간은 어디인가요?': {
      'en': 'Where is the space where you will spend the most time today?',
      'ja': '私が今日一日の中で最も時間を費やすスペースはどこですか？',
    },
    '오늘 내가 다른 사람에게 듣고 싶은 말은 무엇인가요?': {
      'en': 'What do you want to hear from others today?',
      'ja': '今日私が他の人に聞きたいことは何ですか？',
    },
    '나의 일상에서 당연하게 여겼던 것들 중 오늘 감사함을 느끼는 것은?': {
      'en': 'Of the things you took for granted in your daily life, what are you grateful for today?',
      'ja': '私の日常で当然と思っていたもののうち、今日感謝を感じるものは？',
    },
    '내가 오늘 하루를 성공적으로 보냈다고 느킬 수 있는 기준은?': {
      'en': 'What are the criteria for feeling that you spent today successfully?',
      'ja': '私が今日一日をうまく過ごしたと感じる基準は？',
    },
    '오늘 내가 버리고 싶은 마음의 짐이나 고민은?': {
      'en': 'What mental burden or worry do you want to let go of today?',
      'ja': '今日私が捨てたい気持ちの荷物や悩みは？',
    },
    '내가 좋아하는 취미가 오늘 나의 기분에 어떤 영향을 주나요?': {
      'en': 'How does your favorite hobby affect your mood today?',
      'ja': '私の好きな趣味が今日の気分にどのような影響を与えますか？',
    },
    '오늘 내가 새롭게 만날 인연이나 상황에 대한 나의 기대는?': {
      'en': 'What are your expectations for the new relationships or situations you will encounter today?',
      'ja': '今日、私が新たに会う縁や状況に対する私の期待は？',
    },
    '나의 삶을 더욱 풍요롭게 만들어 주는 사소한 취향은?': {
      'en': 'What are the small tastes that make your life richer?',
      'ja': '私の人生をより豊かにしてくれる些細な好みは？',
    },
    '오늘 내가 나 자신을 위해 정성껏 준비하고 싶은 것은?': {
      'en': 'What do you want to prepare carefully for yourself today?',
      'ja': '今日私が自分自身のために丁寧に準備したいことは？',
    },
    '내가 오늘 실천하고 싶은 가장 솔직한 감정 표현은?': {
      'en': 'What is the most honest expression of emotion you want to practice today?',
      'ja': '私が今日実践したい最も率直な感情表現は？',
    },
    '오늘 하루는 어땠나요?': {
      'en': 'How was your day?',
      'ja': '今日の一日はどうでしたか？',
    },
    '오늘 하루는 어떠셨나요?': {
      'en': 'How was your day?',
      'ja': '今日の一日はどうでしたか？',
    },
    '오늘 가장 고마웠던 사람은 누구인가요?': {
      'en': 'Who are you most grateful for today?',
      'ja': '今日最も感謝した人は誰ですか？',
    },
    '오늘 나를 웃게 만든 일은 무엇인가요?': {
      'en': 'What made you smile today?',
      'ja': '今日私を笑わせたことは何ですか？',
    },
    '오늘의 가장 행복했던 순간은 언제였나요?': {
      'en': 'When was the happiest moment today?',
      'ja': '今日の最も幸せな瞬間はいつでしたか？',
    },
    '오늘 배운 점이 있다면 무엇인가요?': {
      'en': 'What did you learn today?',
      'ja': '今日学んだことがあるとしたら？',
    },
    '오늘의 나를 칭찬해주세요.': {
      'en': 'Please give yourself a compliment today.',
      'ja': '今日の私を賞賛してください。',
    },
    '지금 이 순간 나 자신에게 해주고 싶은 칭찬 한마디는?': {
      'en': 'What is one compliment you want to give yourself right now?',
      'ja': '今この瞬間、自分自身にやりたいと思う褒め言葉は？',
    },
    '오늘 가장 힘들었던 일은 무엇인가요?': {
      'en': 'What was the hardest thing today?',
      'ja': '今日一番大変だったことは何ですか？',
    },
    '지금 기분은 어떤가요?': {
      'en': 'How are you feeling right now?',
      'ja': '今の気分はどうですか？',
    },
    '지금 기분이 어떤가요?': {
      'en': 'How are you feeling right now?',
      'ja': '今気分はどうですか？',
    },
    '오늘 하루를 한 단어로 표현한다면?': {
      'en': 'How would you describe today in one word?',
      'ja': '今日一日を一言で表現したら？',
    },
    '오늘 특별한 일이 있었나요?': {
      'en': 'Was there anything special today?',
      'ja': '今日は特別なことがありましたか？',
    },
    '내일 가장 기대되는 일은 무엇인가요?': {
      'en': 'What are you most looking forward to tomorrow?',
      'ja': '明日最も期待されることは何ですか？',
    },
    '오늘 스스로를 위해 한 일은 무엇인가요?': {
      'en': 'What did you do for yourself today?',
      'ja': '今日自分のためにしたことは何ですか？',
    },
    '오늘 마주친 기억에 남는 장면이 있나요?': {
      'en': 'Is there a memorable scene you encountered today?',
      'ja': '今日遭遇した記憶に残るシーンはありますか？',
    },
    '오늘 하루 중 가장 평온했던 시간은 언제인가요?': {
      'en': 'When was the most peaceful time of your day?',
      'ja': '今日の一日の中で最も穏やかな時間はいつですか？',
    },
    '오늘 날씨는 어땠나요?': {
      'en': 'How was the weather today?',
      'ja': '今日の天気はどうでしたか？',
    },
    '오늘 무엇을 먹었나요?': {
      'en': 'What did you eat today?',
      'ja': '今日何を食べましたか？',
    },
    '오늘 가장 기억에 남는 대화는?': {
      'en': 'What was the most memorable conversation today?',
      'ja': '今日最も記憶に残る会話は？',
    },
    '오늘 나의 기분 점수는 10점 만점에 몇 점인가요?': {
      'en': 'On a scale of 1 to 10, how would you rate your mood today?',
      'ja': '今日の私の気分スコアは10点満点で何点ですか？',
    },
    '오늘 아침 나를 미소 짓게 한 아주 작은 발견은?': {
      'en': 'What was the tiny discovery that made you smile this morning?',
      'ja': '今朝私を笑顔にした非常に小さな発見は？',
    },
    '내가 어제보다 오늘 더 성숙해졌다고 느끼는 포인트는?': {
      'en': 'In what way do you feel more mature today than yesterday?',
      'ja': '私が昨日より今日より成熟したと感じるポイントは？',
    },
    '오늘 하루 중 내가 가장 창의적일 수 있는 시간은 언제인가요?': {
      'en': 'When is the time of day you can be the most creative today?',
      'ja': '今日の一日の中で私が最も創造的である時間はいつですか？',
    },
    '내가 요즘 가장 아끼는 노래의 가사 한 구절은?': {
      'en': 'What is a line of lyrics from a song you cherish the most these days?',
      'ja': '私が最近最も大切な曲の歌詞の詩は？',
    },
    '오늘 나를 상쾌하게 만들어 줄 향기는 무엇인가요?': {
      'en': 'What is the scent that will make you feel refreshed today?',
      'ja': '今日私を爽やかにする香りは何ですか？',
    },
    '만약 오늘 내가 주인공인 세상이라면 어떤 일이 일어날까요?': {
      'en': 'If today were a world where I am the main character, what would happen?',
      'ja': 'もし今日私が主人公である世界だとしたらどうなりますか？',
    },
    '내가 최근에 받은 친절 중 가장 기억에 남는 것은?': {
      'en': 'What is the most memorable kindness you received recently?',
      'ja': '私が最近受けた親切の中で最も記憶に残るのは？',
    },
    '오늘 하루 동안 내가 꼭 지키고 싶은 고유한 성격은?': {
      'en': 'What is the unique personality trait you definitely want to keep today?',
      'ja': '今日の一日に私が必ず守りたいユニークな性格は？',
    },
    '내 주변 공간에서 오늘 가장 먼저 정리하고 싶은 곳은?': {
      'en': 'Where in your surrounding space do you want to organize first today?',
      'ja': '私の周りの空間で今日最初に整理したい場所は？',
    },
    '오늘 내가 시도해보고 싶은 새로운 음식이나 맛은?': {
      'en': 'What is a new food or flavor you want to try today?',
      'ja': '今日私が試してみたい新しい食べ物や味は？',
    },
    '내가 가진 것들 중 돈으로 환산할 수 없는 가치는 무엇인가요?': {
      'en': 'What is a value you possess that cannot be converted into money?',
      'ja': '私が持っているもののうち、お金に換算できない価値は何ですか？',
    },
    '오늘 하루 동안 내가 느낄 수 있는 가장 평온한 감정은?': {
      'en': 'What is the most peaceful emotion you can feel throughout the day today?',
      'ja': '今日の日の間に私が感じることができる最も穏やかな感情は？',
    },
    '오늘 아침 거울 속 나의 모습에게 해주고 싶은 말은?': {
      'en': 'What do you want to say to your reflection in the mirror this morning?',
      'ja': '今朝、鏡の中の私の姿にやりたいことは？',
    },
    '내가 최근에 경험한 사소하지만 확실한 행복은?': {
      'en': 'What is a small but certain happiness you experienced recently?',
      'ja': '私が最近経験した些細ですが、確実な幸せは？',
    },
    '오늘 하루 중 내가 가장 에너지가 넘칠 순간은 언제일까요?': {
      'en': 'When will be the moment you are most energetic today?',
      'ja': '今日の一日のうち、私が最もエネルギーがあふれた瞬間はいつでしょうか？',
    },
    '오늘 내가 만날 사람들에게 어떤 인상을 남기고 싶나요?': {
      'en': 'What kind of impression do you want to leave on the people you meet today?',
      'ja': '今日、私が会う人たちにどんな印象を残したいですか？',
    },
    '오늘 하루를 마치고 나를 위해 준비하고 싶은 보상은 무엇인가요?': {
      'en': 'What is the reward you want to prepare for yourself after finishing today?',
      'ja': '今日一日を終えて私のために準備したい報酬は何ですか？',
    },
    '오늘 하루 동안 내가 감사함을 표현할 수 있는 대상 3가지는?': {
      'en': 'What are three things you can express gratitude for throughout the day today?',
      'ja': '今日の日に私が感謝の気持ちを表現できる3つの目標は何ですか？',
    },
    '만약 오늘 하루만 다른 사람이 될 수 있다면 누구의 삶을 살아볼까요?': {
      'en': 'If I could be someone else for just today, whose life would I live?',
      'ja': 'もし今日一日だけ他人になれたら誰の人生を生きてみましょうか？',
    },
    '오늘 아침 내가 마신 물이나 차의 온도는 어떠했나요?': {
      'en': 'How was the temperature of the water or tea you drank this morning?',
      'ja': '今朝私が飲んだ水やお茶の気温はどうでしたか？',
    },
    '내가 오늘 하루 동안 꼭 실천하고 싶은 건강한 습관 하나는?': {
      'en': 'What is one healthy habit you definitely want to practice today?',
      'ja': '私が今日の日に必ず実践したい健康的な習慣の一つは？',
    },
    '오늘 내가 마주할 풍경 속에 숨겨진 아름다움을 찾아본다면?': {
      'en': 'If you were to find the hidden beauty in the landscape you will encounter today?',
      'ja': '今日私が向かい合う風景の中に隠された美しさを探していたら？',
    },
    '내가 최근에 배운 지식 중 다른 사람에게 공유하고 싶은 것은?': {
      'en': 'Of the knowledge you recently learned, what do you want to share with others?',
      'ja': '私が最近学んだ知識の中で他人に共有したいことは？',
    },
    '오늘 하루 동안 내가 피곤함을 느낄 때 나를 위도해줄 문구는?': {
      'en': 'What is a phrase that will comfort you when you feel tired during the day today?',
      'ja': '今日の一日で私が疲れていると感じたときに私を慰めるフレーズは？',
    },
    '내가 오늘 완성하고 싶은 가장 작은 성취는 무엇인가요?': {
      'en': 'What is the smallest achievement you want to complete today?',
      'ja': '私が今日完成したい最小の達成は何ですか？',
    },
    '오늘 하루 중 내가 가장 진솔해질 수 있는 순간은 언제인가요?': {
      'en': 'When is the moment you can be the most sincere today?',
      'ja': '今日の一日中私が最も率直することができる瞬間はいつですか？',
    },
    '오늘 아침 나를 깨운 소리는 무엇이었나요?': {
      'en': 'What was the sound that woke you up this morning?',
      'ja': '今朝私を目覚めた音は何でしたか？',
    },
    '내가 요즘 가장 흥미를 느끼는 기술이나 문화는 무엇인가요?': {
      'en': 'What is the technology or culture you are most interested in these days?',
      'ja': '私が最近最も興味を持っている技術や文化は何ですか？',
    },
    '오늘 하루 동안 내가 마주할 우연한 일들에 대한 나의 생각은?': {
      'en': 'What are your thoughts on the accidental things you will encounter today?',
      'ja': '今日の一日に私が遭遇する偶然のことについての私の考えは？',
    },
    '내가 오늘 스스로에게 허락하고 싶은 아주 사소한 일탈은?': {
      'en': 'What is a very small deviation you want to allow yourself today?',
      'ja': '私が今日自分に許したいとても些細な逸脱は？',
    },
    '오늘 하루 중 내가 가장 고마움을 느낄 대상은 누구일까요?': {
      'en': 'Who will be the person you feel the most grateful to today?',
      'ja': '今日の一日中、私が最も感謝を感じる対象は誰ですか？',
    },
    '오늘 아침 내가 느낀 공기의 상쾌함은 어떠했나요?': {
      'en': 'How was the freshness of the air you felt this morning?',
      'ja': '今朝私が感じた空気のさわやかさはどうでしたか？',
    },
    '오늘 하루 동안 내가 집중하고 싶은 나의 강점 하나는?': {
      'en': 'What is one of your strengths you want to focus on throughout the day today?',
      'ja': '今日の日に私が集中したい私の強みの一つは？',
    },
    '오늘 하루를 보내며 가장 많이 사용하게 될 물건은 무엇인가요?': {
      'en': 'What is the object you will use the most while spending today?',
      'ja': '今日一日を過ごし、最もよく使われるものは何ですか？',
    },
    '오늘 하루 중 내가 가장 평화롭게 느낄 장소는 어디인가요?': {
      'en': 'Where is the place you will feel the most peaceful today?',
      'ja': '今日の一日の中で私が最も平和に感じる場所はどこですか？',
    },
    '만약 오늘 새로운 취미를 시작한다면 무엇이 좋을까요?': {
      'en': 'If I were to start a new hobby today, what would be good?',
      'ja': '今日の新しい趣味を始めたら、何がいいでしょうか？',
    },
    '오늘 아침 내가 본 첫 번째 이미지나 풍경은 무엇이었나요?': {
      'en': 'What was the first image or landscape you saw this morning?',
      'ja': '今朝私が見た最初のイメージや風景は何でしたか？',
    },
    '내가 요즘 가장 많이 생각하는 미래의 나의 모습은?': {
      'en': 'What is the image of my future self that I think about the most these days?',
      'ja': '私が最近最も考えている未来の私の姿は？',
    },
    '오늘 하루 동안 내가 마주할 도전들을 어떻게 즐길 수 있을까요?': {
      'en': 'How can I enjoy the challenges I will face throughout the day today?',
      'ja': '今日の日中、私が向き合う挑戦をどのように楽しむことができますか？',
    },
    '오늘 하루를 마칠 때 나 자신에게 해주고 싶은 칭찬은?': {
      'en': 'What is the compliment you want to give yourself when you finish today?',
      'ja': '今日一日を終えたときに自分にしてほしい賞賛は？',
    },
    '오늘 하루 중 내가 가장 기대하는 만남이나 이벤트는 무엇인가요?': {
      'en': 'What is the meeting or event you are most looking forward to today?',
      'ja': '今日の一日のうち、私が最も期待している出会いやイベントは何ですか？',
    },
    '만약 오늘 하루만 자유롭게 여행할 수 있다면 어디로 갈까요?': {
      'en': 'If I could travel freely for just today, where would I go?',
      'ja': '今日一日だけ自由に旅行できるならどこに行きますか？',
    },
    '오늘 하루가 나에게 줄 가장 큰 선물은 무엇일까요?': {
      'en': 'What will be the biggest gift today gives to me?',
      'ja': '今日の一日が私に与える最大の贈り物は何ですか？',
    },
    '오늘 나의 마음가짐을 한 문장의 좌우명으로 정한다면?': {
      'en': 'If you were to set your mindset today as a one-sentence motto?',
      'ja': '今日私の気持ちを一文のモットーで決めたら？',
    },
    '내가 오늘 누군가에게 전하고픈 응원의 한마디는?': {
      'en': 'What is a word of encouragement you want to give to someone today?',
      'ja': '私が今日誰かに伝えた応援の一言は？',
    },
    '오늘 하루 동안 내 마음을 가장 따뜻하게 해줄 추억은?': {
      'en': 'What is the memory that will warm your heart the most today?',
      'ja': '今日の一日の中で私の心を最も暖かくしてくれる思い出は？',
    },
    '내가 오늘 꼭 해결하고 싶은 사소한 고민거리는 무엇인가요?': {
      'en': 'What is a small worry you definitely want to resolve today?',
      'ja': '私が今日是非解決したい些細な悩みは何ですか？',
    },
    '오늘 하루 중 내가 가장 나다워지는 순간은 언제인가요?': {
      'en': 'When is the moment you feel most like yourself today?',
      'ja': '今日の一日の中で一番気になる瞬間はいつですか？',
    },
    '내가 오늘 새롭게 발견하고 싶은 나의 또 다른 모습은?': {
      'en': 'What is another side of yourself you want to discover today?',
      'ja': '私が今日新たに発見したい私のもう一つの外観は？',
    },
    '오늘 하루가 끝났을 때 내가 가장 뿌듯해할 일은?': {
      'en': 'What will you be most proud of when today is over?',
      'ja': '今日の一日が終わったときに私が最も嬉しいことは何ですか？',
    },
  };

  // 모든 질문을 DB에 초기화/업데이트하는 함수 (100개+) - updateQuestionTranslations에 통합됨
  // Future<void> seedQuestions() async { ... }

  Future<void> updateJapaneseAssetNames() async {
    _isLoading = true;
    notifyListeners();
    try {
      debugPrint('==== 일본어 에셋 이름 업데이트 ====');
    } catch (e) {
      debugPrint('일본어 에셋 이름 업데이트 오류: \$e');
    }
    _isLoading = false;
    notifyListeners();
  }

  Future<void> updateQuestionTranslations() async {
    _isLoading = true;
    notifyListeners();

    try {
      final dbQuestions = await _questionService.getAllQuestions();
      final int initialDbCount = dbQuestions.length;

      debugPrint('==== 질문 동기화 및 번역 시작 (DB 기존: $initialDbCount개) ====');

      int updatedCount = 0;
      int addedCount = 0;

      String normalize(String text) {
        return text.replaceAll(RegExp(r'[\s\?\!.,]'), '');
      }

      for (var entry in questionTranslationMap.entries) {
        final String masterKoreanText = entry.key;
        final String masterEngText = entry.value['en'] ?? '';
        final String masterJaText = entry.value['ja'] ?? '';
        final String normalizedMasterKey = normalize(masterKoreanText);

        bool existsInDb = false;
        String? existingDocId;

        for (var dbQ in dbQuestions) {
          final String normalizedDbText = normalize(dbQ.text);
          if (normalizedDbText == normalizedMasterKey ||
              normalizedDbText.contains(normalizedMasterKey) ||
              normalizedMasterKey.contains(normalizedDbText)) {
            existsInDb = true;
            existingDocId = dbQ.id;
            break;
          }
        }

        if (existsInDb && existingDocId != null) {
          await _questionService.updateQuestionTranslation(
              existingDocId, engText: masterEngText, jaText: masterJaText);
          updatedCount++;
        } else {
          await _questionService.addOrUpdateQuestion(
            text: masterKoreanText,
            category: 'daily',
            engText: masterEngText,
            jaText: masterJaText,
          );
          addedCount++;
        }
      }

      debugPrint('결과 리포트:');
      debugPrint('- 번역 업데이트됨: $updatedCount개');
      debugPrint('- 신규 추가됨: $addedCount개');
      debugPrint('==== 질문 동기화 및 번역 종료 ====');
    } catch (e) {
      debugPrint('질문 번역/동기화 업데이트 오류: $e');
    }

    _isLoading = false;
    notifyListeners();
  }  Future<void> syncAllFriendData() async {
    _isLoading = true;
    notifyListeners();
    try {
      final usersSnapshot = await _firestore.collection('users').get();
      int updatedCount = 0;

      for (var userDoc in usersSnapshot.docs) {
        final userId = userDoc.id;

        final friendsSnapshot = await _firestore
            .collection('friends')
            .where('userId', isEqualTo: userId)
            .where('status', isEqualTo: 'accepted')
            .get();

        Set<String> validFriendIds = {};
        for (var fDoc in friendsSnapshot.docs) {
          validFriendIds.add(fDoc['friendId'] as String);
        }

        await userDoc.reference.update({'friendIds': validFriendIds.toList()});
        updatedCount++;
      }

      debugPrint('친구 데이터 동기화 완료: $updatedCount명 업데이트됨.');
    } catch (e) {
      debugPrint('친구 데이터 동기화 오류: $e');
    }

    _isLoading = false;
    notifyListeners();
  }

  // migrateAssetsToFirestore was replaced by syncShopAssets for better consistency

  Future<List<String>> scanLocalDiaries(String targetUserId) async {
    List<String> foundDates = [];
    try {
      final directory = await getApplicationDocumentsDirectory();
      final files = directory.listSync();

      for (var file in files) {
        if (file is File) {
          final filename = file.uri.pathSegments.last;
          if (filename.startsWith('${targetUserId}_') &&
              filename.endsWith('.enc')) {
            final datePart = filename.substring(
                targetUserId.length + 1, filename.length - 4);
            foundDates.add(datePart);
          }
        }
      }
      foundDates.sort((a, b) => b.compareTo(a));
    } catch (e) {
      debugPrint('로컬 일기 스캔 오류: $e');
    }
    return foundDates;
  }

  Future<bool> recoverLocalDiary(String targetUserId, String dateStr) async {
    try {
      _isLoading = true;
      notifyListeners();

      int year, month, day;

      if (dateStr.contains('-')) {
        final dateParts = dateStr.split('-');
        if (dateParts.length != 3) {
          _isLoading = false;
          notifyListeners();
          return false;
        }
        year = int.parse(dateParts[0]);
        month = int.parse(dateParts[1]);
        day = int.parse(dateParts[2]);
      } else if (dateStr.length == 8) {
        year = int.parse(dateStr.substring(0, 4));
        month = int.parse(dateStr.substring(4, 6));
        day = int.parse(dateStr.substring(6, 8));
      } else if (dateStr.length >= 6) {
        year = int.parse(dateStr.substring(0, 4));
        if (dateStr.length == 6) {
          month = int.parse(dateStr.substring(4, 5));
          day = int.parse(dateStr.substring(5, 6));
        } else {
          final m2 = int.parse(dateStr.substring(4, 6));
          if (m2 <= 12) {
            month = m2;
            day = int.parse(dateStr.substring(6));
          } else {
            month = int.parse(dateStr.substring(4, 5));
            day = int.parse(dateStr.substring(5));
          }
        }
      } else {
        _isLoading = false;
        notifyListeners();
        return false;
      }

      final directory = await getApplicationDocumentsDirectory();

      final filename1 = '${targetUserId}_$dateStr.enc';
      final pathSeparator = Platform.isWindows ? '\\' : '/';

      final filePath1 = '${directory.path}$pathSeparator$filename1';
      final file1 = File(filePath1);

      // Fallback for when dateStr contains '-' and we want to try the parsed digits
      final filename2 =
          '${targetUserId}_${year}${month.toString().padLeft(2, '0')}${day.toString().padLeft(2, '0')}.enc';
      final filePath2 = '${directory.path}$pathSeparator$filename2';
      final file2 = File(filePath2);

      // Fallback for unpadded parsed digits
      final filename3 = '${targetUserId}_$year$month$day.enc';
      final filePath3 = '${directory.path}$pathSeparator$filename3';
      final file3 = File(filePath3);

      File targetFile;
      if (await file1.exists()) {
        targetFile = file1;
      } else if (await file2.exists()) {
        targetFile = file2;
      } else if (await file3.exists()) {
        targetFile = file3;
      } else {
        _isLoading = false;
        notifyListeners();
        return false;
      }

      final encryptedContent = await targetFile.readAsString();

      final customDate = DateTime(year, month, day);
      final dateKey = DiaryModel.buildDateKey(customDate);

      final query = await _firestore
          .collection('diaries')
          .where('userId', isEqualTo: targetUserId)
          .where('dateKey', isEqualTo: dateKey)
          .limit(1)
          .get();

      String? existingId;
      if (query.docs.isNotEmpty) {
        existingId = query.docs.first.id;
      }

      final decrypted =
          await EncryptionUtil.decryptText(encryptedContent, targetUserId);

      final diary = DiaryModel(
        id: existingId ?? '',
        userId: targetUserId,
        date: customDate,
        dateKey: dateKey,
        encryptedContent: encryptedContent,
        wordCount: decrypted.length,
        writingDuration: 0,
        moods: ['normal'],
        isCompleted: true,
        createdAt: DateTime.now(),
      );

      if (existingId != null && existingId.isNotEmpty) {
        await _firestore
            .collection('diaries')
            .doc(existingId)
            .update(diary.toFirestore());
      } else {
        await _firestore.collection('diaries').add(diary.toFirestore());
      }

      _isLoading = false;
      notifyListeners();
      return true;
    } catch (e) {
      debugPrint('로컬 일기 복구 오류: $e');
    }
    _isLoading = false;
    notifyListeners();
    return false;
  }
}
